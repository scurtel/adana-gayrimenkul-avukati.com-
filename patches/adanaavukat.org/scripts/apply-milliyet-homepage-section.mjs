#!/usr/bin/env node
/**
 * Ana sayfaya Milliyet Gazetesi hukuk yazıları bölümünü additive ekler.
 *
 ...
 *
 * Gizli değerler (kullanıcı adı, Application Password) loglanmaz.
 * Schema / Rank Math / canonical / H1 / mevcut H2 / header / footer dokunulmaz.
 */
...
const HOMEPAGE_ID = 7;
const PUBLIC_URL = 'https://adanaavukat.org/';
const AUTH_TEST = process.argv.includes('--auth-test');
const EXECUTE = process.argv.includes('--execute');
const EXPECTED_INTRO =
  'Av. Ceren Sümer Cilli’nin Milliyet Gazetesi’nde yayımlanan aile hukuku ve özel hukuk alanındaki yazılarından seçmeler.';
const EXPECTED_MILLIYET_URLS = MILLIYET_ARTICLES.map((a) => a.url);
const PRE_H2 = [
  'Hizmet Alanları',
  'Av. Ceren Sümer Cilli',
  'Neden Hukuki Destek Önemli?',
  'Hukuk Rehberi — Son Yazılar',
  'Sık Sorulan Sorular',
  'Hukuki Sürecinizi Değerlendirmek İçin İletişime Geçin',
];
...
function authHeader() {
  const { username, appPassword } = credStatus();
  return getAuthHeader(username, appPassword);
}

async function wpRequest(path, { method = 'GET', body } = {}) {
  const { baseUrl } = credStatus();
...
async function fetchPublicHtml() {
  const res = await fetch(`${PUBLIC_URL}?nocache=${Date.now()}`, {
    headers: {
      ...
      'User-Agent': 'adanaavukat-milliyet-section',
    },
  });
...
function publicSignals(html, status) {
  ...
  const sectionIdCount =
    ...
    (html.match(/id='milliyet-hukuk-yazilari'/g) || []).length;
  const h2Count = h2.filter((t) => t.includes(normalizeText(SECTION_H2).replace(/'/g, "'")) || normalizeText(t) === normalizeText(SECTION_H2)).length;
  return {
    status,
    ...
    hasAaGrid: html.includes('aa-grid-3') && html.includes('aa-card'),
...
function verifyAgainstBaseline(before, after) {
  ...
  add(2, after.sectionIdCount === 1 && after.milliyetH2Count === 1, `Bölüm bir kez eklenmiş (id=${after.sectionIdCount}, h2=${after.milliyetH2Count})`);
  ...
  add(5, missingArticles.length === 0 && after.milliyetLinkCount === 6, `6 Milliyet bağlantısı (bulunan ${after.milliyetLinkCount})`);
  add(6, true, 'adanabosanmaavukati.org kontrolü bu sitede uygulanmaz');
  add(7, true, 'anlaşmalı/çekişmeli/mal paylaşımı bu sitede zorunlu değil');
  ...
  add('render', after.hasAaGrid, 'Mevcut aa-grid-3 / aa-card sınıfları kullanılıyor (desktop/mobil grid)');
...
function writeBackup(dir, raw, publicHtml, meta) {
  ...
  const publicFile = resolve(dir, `homepage-${HOMEPAGE_ID}-pre-milliyet-${ts}.public.html`);
  const metaFile = resolve(dir, `homepage-${HOMEPAGE_ID}-pre-milliyet-${ts}.meta.json`);
  writeFileSync(rawFile, raw, 'utf8');
  writeFileSync(publicFile, publicHtml, 'utf8');
  writeFileSync(metaFile, JSON.stringify(meta, null, 2), 'utf8');
  return { rawFile, publicFile, metaFile };
}

async function updatePageContent(content) {
  return wpRequest(`/wp-json/wp/v2/pages/${HOMEPAGE_ID}`, {
    method: 'POST',
    body: { content },
  });
}
...
async function authTest() {
  ...
  console.log('=== AUTH TEST — adanaavukat.org page 7 ===');
  console.log(`baseUrl: ${creds.baseUrl}`);
  console.log(`ADANAAVUKAT_WP_USERNAME: ${creds.usernamePresent ? 'PRESENT' : 'MISSING'}`);
  console.log(`ADANAAVUKAT_WP_APP_PASSWORD: ${creds.appPasswordPresent ? 'PRESENT' : 'MISSING'}`);
...
async function main() {
  ...
  if (!EXECUTE) {
    ...
    writeFileSync(resolve(rootDir, 'generated/homepage-milliyet-section-preview.html'), result.html, 'utf8');
  ...
  const backup = writeBackup(backupDir, raw, publicBefore.html, {
    pageId: HOMEPAGE_ID,
